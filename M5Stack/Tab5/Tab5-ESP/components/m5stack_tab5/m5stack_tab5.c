...
static esp_err_t bsp_gt911_force_wake(void)
{
    const gpio_config_t int_gpio_high = {
        .mode = GPIO_MODE_OUTPUT,
        .pull_up_en = GPIO_PULLUP_ENABLE,
        .pin_bit_mask = BIT64(BSP_LCD_TOUCH_INT),
    };
    ESP_RETURN_ON_ERROR(gpio_config(&int_gpio_high), TAG, "GT911 INT high config failed");
    ESP_RETURN_ON_ERROR(gpio_set_level(BSP_LCD_TOUCH_INT, 1), TAG, "GT911 INT high level failed");
    vTaskDelay(pdMS_TO_TICKS(20));

    const gpio_config_t int_gpio_input = {
    ...
    ESP_RETURN_ON_ERROR(gpio_config(&int_gpio_input), TAG, "GT911 INT input config failed");
    vTaskDelay(pdMS_TO_TICKS(20));

    ESP_LOGI(TAG, "GT911 INT forced high to keep controller awake");
    return ESP_OK;
}
...
static lv_indev_t* bsp_display_indev_init(lv_display_t* disp)
{
    ...
    BSP_ERROR_CHECK_RETURN_NULL(bsp_gt911_force_wake());
    assert(tp);
    ...
    if (indev == NULL) {
        ESP_LOGE(TAG, "Failed to add GT911 touch to LVGL");
        return NULL;
    }

    lv_indev_set_mode(indev, LV_INDEV_MODE_TIMER);

    ESP_LOGI(TAG, "GT911 touch panel initialized successfully in polling mode");
    return indev;
}
