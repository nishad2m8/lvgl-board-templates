...
// Select which ESP-IDF I2C driver API to use.
//  - Define I2C_BSP_FORCE_NEW_DRIVER to keep using esp-idf v5's new i2c_master API.
//  - Define I2C_BSP_FORCE_LEGACY_DRIVER to force the legacy driver regardless of framework.
//  - When building under Arduino we default to the legacy driver to avoid the
//    `check_i2c_driver_conflict` abort that happens if both drivers are linked.
#if defined(I2C_BSP_FORCE_LEGACY_DRIVER)
#define I2C_BSP_USE_LEGACY_DRIVER 1
#elif defined(I2C_BSP_FORCE_NEW_DRIVER)
#define I2C_BSP_USE_LEGACY_DRIVER 0
...
#if !I2C_BSP_USE_LEGACY_DRIVER
// I2C Master Bus Handle (new driver path only)
