...
epaper_driver_display::epaper_driver_display(int width, int height,custom_lcd_spi_t _lcd_spi_data) :
    ...
    ESP_LOGI(TAG, "Allocating buffer: %d bytes", lcd_spi_data.buffer_len);
    // Try SPIRAM first for better performance
    ...
    if (buffer == NULL) {
        ...
        // Fall back to regular heap if SPIRAM allocation fails
        buffer = (uint8_t *)heap_caps_malloc(lcd_spi_data.buffer_len, MALLOC_CAP_8BIT);
    }

    if (buffer != NULL) {
        ESP_LOGI(TAG, "Buffer allocated successfully at %p", buffer);
    } else {
        ESP_LOGE(TAG, "Buffer allocation failed! Requested size: %d bytes", lcd_spi_data.buffer_len);
    }
	assert(buffer);
}
